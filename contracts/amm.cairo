...
struct AmmState:
    ...
    # Pointer to the beginning of the AMM
    member account_dict_start : DictAccess*
    # Pointer to the end of the AMM
...
func modify_account{range_check_ptr}(state : AmmState, account_id, diff_a, diff_b) -> (
    ...
    let (local old_account : AmmAccount*) = dict_read{dict_ptr=account_dict_end}(key=account_id)

    #Increase all balance
    ...
    # assert is used in order to attribute all new value
    # WARNING : assert is to attribute value and assert_le is a test condition
    local new_account : AmmAccount
    assert new_account.account_id = old_account.account_id
    ...
    # Get the frame pointer and programm counter
...
func swap{range_check_ptr}(state : AmmState, transaction : SwapTransaction*) -> (state : AmmState):
    ...
    #   b = (y * a) / (x + a).
    # First return element in the quotient and the second one the remainder
    ...
    tempvar new_x = x + a
    tempvar new_y = y - b
    # Check the liquidity amount in the pool after the swap
    ...
    %{
        ...
        # debugging purposes.
        # You can retrieve the value in hints (Python code) be doing ids.
        print(
...
#Use this funciton to do more than found trasnaction at the same time 
# Use SwapTransaction** because we want a pointer of SwapTransaction in order 
# To iterate through the SwapTransaction* list and swap function take a 
# Transaction* parameter
func transaction_loop{range_check_ptr}(
    state : AmmState, transactions : SwapTransaction**, n_transactions : felt
) -> (state : AmmState):
    # Check if number is different to 0 (uint so always positive)
    if n_transactions == 0:
        return (state=state)
    end

    # Use of bracket in order to get the value 
    let first_transaction : SwapTransaction* = [transactions]
    let (state) = swap(state=state, transaction=first_transaction)

    # Tail recursion more optimize
...
# following formula:
#   H(H(account_id, token_a_balance), token_b_balance).
# where H is the Pedersen hash function.
func hash_account{pedersen_ptr : HashBuiltin*}(account : AmmAccount*) -> (res : felt):
    let res = account.account_id
...
# Iterate on it like a Linked List 
func hash_dict_values{pedersen_ptr : HashBuiltin*}(
    ...
    if dict_start == dict_end:
        # Like in C when you finish to modifiy all value, point to the start
        return (hash_dict_end=hash_dict_start)
    end

    # Compute the hash of the account before and after the
    # change.
    let (prev_hash) = hash_account(account=cast(dict_start.prev_value, AmmAccount*))
    let (new_hash) = hash_account(account=cast(dict_start.new_value, AmmAccount*))
    ...
    # DictAccess.SIZE is like go to the next slot since all the dict memory are
    # contiguous
...
func compute_merkle_roots{pedersen_ptr : HashBuiltin*, range_check_ptr}(state : AmmState) -> (
    ...
    %{
        ...
        for account_id, account in initial_account_dict.items():
            account_id = memory[
                account + ids.AmmAccount.account_id]
            ...
            initial_dict[account_id] = pedersen_hash(
                pedersen_hash(account_id, token_a_balance),
                token_b_balance)
    %}
    ...
    let (root_before, root_after) = small_merkle_tree_update{hash_ptr=pedersen_ptr}(
        squashed_dict_start=hash_dict_start, squashed_dict_end=hash_dict_end, height=N_LOGN_ACCOUNT
    )
